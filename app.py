...
def validar():
    ...
    if user:
        if user.get("rol") == "maestro":
            ...
            # IMPORTANTE: Pasamos horarios_col para que el alumno pueda leer la tabla
...
def guardar_horario():
    ...
    for dia in dias:
        ...
        for i in range(1, 18):
            ...
            if valor: # Solo guarda si el maestro seleccionó algo
                bloques[f"bloque_{i}"] = valor
        
        if bloques:
            # Actualizamos sin borrar lo de otros maestros ($set en campos específicos)
            update_data = {f"bloques.{k}": v for k, v in bloques.items()}
            horarios_col.update_one({"cuatrimestre": cuatri, "dia": dia}, {"$set": update_data}, upsert=True)
